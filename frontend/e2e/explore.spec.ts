...
test("the default plan, its band chart and attribution are shown", async ({ page }) => {
  await openExplore(page);
  await expect(page.getByTestId("result-lead")).toContainText("126,330");
  await expect(page.locator("aside").getByTestId("band-row")).toHaveCount(5);
  await expect(page.getByTestId("band-sentence")).toContainText("71,309");
...
test("selecting a stop focuses it and lists its neighbourhoods", async ({ page }) => {
  ...
  await expect(card.getByTestId("neighbourhood-row")).toHaveCount(14);
});
...
test("changing the walking time re-runs the optimiser", async ({ page }) => {
  ...
  await expect(page.getByTestId("result-lead")).not.toContainText("126,330");
});
