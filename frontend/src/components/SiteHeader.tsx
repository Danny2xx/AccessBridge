  ...
  return (
    <span className="grid size-9 place-items-center rounded-lg bg-primary/12 ring-1 ring-primary/30">
      <svg viewBox="0 0 24 24"className="size-5"aria-hidden="true"fill="none">
        <path
          d="M4 17.5 L9.5 9 L14.5 14 L20 5.5"stroke="var(--primary)"strokeWidth="2.4"strokeLinecap="round"strokeLinejoin="round"
        />
...
export function SiteHeader({ route, progress = null }: { route: Route; progress?: number | null }) {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border bg-background/80 ">
      <a
        href="#main"className="absolute top-2 left-3 z-50 -translate-y-20 rounded-full bg-primary px-4 py-2.5 font-bold text-primary-foreground transition-transform focus:translate-y-0"
      >
        Skip to content
      </a>
      ...
      <div className="mx-auto flex h-16 max-w-[110rem] items-center gap-4 px-[clamp(1rem,3vw,2rem)]">
        <a href="#/story"className="flex items-center gap-3 no-underline"aria-label="AccessBridge AI, back to the story">
          ...
          <span className="grid leading-tight">
            <span className="text-[1.05rem] font-semibold tracking-tight">AccessBridge AI</span>
        ...
        <nav aria-label="Sections"className="mx-auto max-md:hidden">
          <ul className="flex items-center gap-1 rounded-full border border-border bg-card p-1.5">
            {NAV.map((item) => {
                  ...
                  >
                    {active ? (
                      <motion.span
                        ...
                        className="absolute inset-0 rounded-full bg-primary"transition={{ type: "spring", stiffness: 450, damping: 38 }}
                      />
                    ) : null}
      ...
      {progress !== null ? (
        <motion.span
          aria-hidden="true"className="absolute inset-x-0 bottom-[-1px] h-0.5 origin-left bg-primary"initial={false}
