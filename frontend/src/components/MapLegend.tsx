...
function legendItems(
  ...
  if (mode === "need")
    BANDS.forEach((band) => items.push({ color: palette.needRamp[band.step], label: band.label }));
  if (mode === "gap") {
    items.push(
  ...
  if (showNetwork)
    items.push({ color: palette.proposal, label: "New stop, numbered along the route", shape: "number" });
...
export function MapLegend({
  ...
  return (
    <div
      ...
      className={`floating pointer-events-none absolute z-10 max-w-[18rem] rounded-md border border-border p-3 text-[0.8125rem] leading-snug ${className}`}
    >
      <p className="mb-2 font-semibold">{TITLES[mode]}</p>
      <ul className="grid gap-1.5 text-muted-foreground">
        {items.map((item) => (
          <li key={item.label} className="grid grid-cols-[1.125rem_1fr] items-center gap-2">
            {item.shape === "number" ? (
              <span
                className="grid size-[1.125rem] place-items-center rounded-full text-[0.6875rem] font-semibold text-primary-foreground"
