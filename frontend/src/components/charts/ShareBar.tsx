...
export function ShareBar({ segments, unit = "people" }: { segments: ShareSegment[]; unit?: string }) {
  ...
  return (
    <div data-chart-body className="relative min-w-0">
      <motion.div
        className="flex h-6 gap-0.5 overflow-hidden rounded-md"initial={reduced ? false : { opacity: 0 }}
        animate={{ opacity: 1 }}
        ...
        {segments
          ...
          .map((segment) => (
            <motion.span
              key={segment.key}
              tabIndex={0}
              role="img"aria-label={`${segment.label}: ${formatNumber(segment.value)} ${unit}, ${percent(segment.value, total)}`}
              style={{ flexGrow: segment.value, background: segment.color }}
              className="min-w-1 basis-0 outline-none transition-[filter] hover:brightness-115 focus-visible:brightness-115 focus-visible:ring-[3px] focus-visible:ring-ring/50 focus-visible:ring-inset"initial={reduced ? false : { scaleY: 0.3 }}
              animate={{ scaleY: 1 }}
        ...
        {segments.map((segment) => (
          <li key={segment.key} className="grid grid-cols-[0.75rem_1fr_auto] items-center gap-2.5">
            ...
            <span className="font-bold tabular">
              {formatNumber(segment.value)} <small className="ml-1.5 font-normal text-dim">{percent(segment.value, total)}</small>
            </span>
          </li>
        ))}
      </ul>
